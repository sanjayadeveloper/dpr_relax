...
// $currentDate
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) $allQryVls ORDER BY a.id DESC $limit");
    ...
    if ($bnsQry_results>0) {
        ...
        while($rows=mysqli_fetch_object($bnsQry)){
            ...
            // resignation check
            $reg_qry = mysqli_query($con,"SELECT * FROM fin_fnf WHERE emp='$rows->empIds' AND closing_status!='5' ORDER BY id DESC LIMIT 1");
            $reg_row = mysqli_num_rows($reg_qry);
            if ($reg_row==0) {

                // SELECT a.id as ids,a.created_on as created_at,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) AND c.id IN (SELECT emp FROM `fin_fnf`WHERE closing_status!='5' ORDER BY `id` DESC) ORDER BY a.id DESC

                // SELECT a.id as ids,a.created_on as created_at,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) AND c.id IN (SELECT MAX(id) FROM `fin_fnf` WHERE closing_status!='5'AND emp=c.id) ORDER BY a.id DESC
...
if ($numAddBonusReq==2) {
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) AND a.b_status='1' $allQryVls ORDER BY a.id DESC $limit");
    $bnsQry_results = mysqli_num_rows($bnsQry);
    if ($bnsQry_results>0) {
        $counter=$start;
        while($rows=mysqli_fetch_object($bnsQry)){
...
if ($numAddBonusReq==3) {
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) $allQryVls AND a.b_status='6' ORDER BY a.id DESC $limit");
    $bnsQry_results = mysqli_num_rows($bnsQry);
    if ($bnsQry_results>0) {
        $counter=$start;
        while($rows=mysqli_fetch_object($bnsQry)){
...
if ($numManageBonusReq==1) {
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.*,d.id as orgId FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) AND (a.b_status='0' || a.b_status='2') $mngQryVls ORDER BY a.id DESC $limit");
    $bnsQry_results = mysqli_num_rows($bnsQry);
    if ($bnsQry_results>0) {
        $counter=$start;
        while($rows=mysqli_fetch_object($bnsQry)){
...
if ($numManageBonusReq==2) {
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.* FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) $mngQryVls AND a.b_status='3' ORDER BY a.id DESC $limit");
    $bnsQry_results = mysqli_num_rows($bnsQry);
    if ($bnsQry_results>0) {
        $counter=0;
        while($rows=mysqli_fetch_object($bnsQry)){
...
if ($numManageBonusReq==3) {
    ...
    $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.created_on as created_at,a.created_by as crtd_by,a.*,b.*,c.*,c.id as empIds,d.* FROM hr_bonus_request a, master_type_dtls b, mstr_emp c, prj_organisation d WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.org_id=d.id AND a.b_id IN (SELECT id FROM `hr_bonus` WHERE b_status=1) $mngQryVls AND a.b_status='4' ORDER BY a.id DESC $limit");
    $bnsQry_results = mysqli_num_rows($bnsQry);
    if ($bnsQry_results>0) {
        $counter=0;
        while($rows=mysqli_fetch_object($bnsQry)){
