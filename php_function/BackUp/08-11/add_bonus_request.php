...
if ($action=='orgNameCheck') {
    ...
    if ($orgNameVls!='' || $orgNameVls!=null) {
        ...
        if ($empName!='0') {
            ...
            $qry = "SELECT x.id as empID, y.fullname as name, y.designation as desig, a.dept_name as deptname, b.lname as locname FROM hr_employee_service_register x, mstr_emp y, hr_department a, hr_location b WHERE ref_id = 0 AND x.emp_name = y.id AND y.status='1' AND x.department_id='$orgNameVls' AND x.department_id=a.id AND x.location_id=b.id UNION (SELECT x.id as empID, x.emp_name as name, y.designation as desig, a.dept_name as deptname, b.lname as locname FROM hr_employee_service_register x, mstr_emp y, hr_department a, hr_location b WHERE ref_id <> 0 AND y.mstr_ref_id = x.ref_id AND y.status='1' AND x.department_id='$orgNameVls' AND x.department_id=a.id AND x.location_id=b.id)";
        }
