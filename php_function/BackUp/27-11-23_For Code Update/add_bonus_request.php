...
if ($action=='orgNameCheck') {
    ...
    if ($orgNameVls!='' || $orgNameVls!=null) {
        ...
        if ($empName!='0') {
            ...
            $qry = "SELECT x.id as empID, y.fullname as name, y.designation as desig, a.dept_name as deptname, b.lname as locname FROM hr_employee_service_register x, mstr_emp y, hr_department a, hr_location b WHERE ref_id = 0 AND x.emp_name = y.id AND y.status='1' AND x.org_nm='$orgNameVls' AND y.fullname LIKE '%$empName%' AND x.department_id=a.id AND x.location_id=b.id UNION (SELECT x.id as empID, x.emp_name as name, y.designation as desig, a.dept_name as deptname, b.lname as locname FROM hr_employee_service_register x, mstr_emp y, hr_department a, hr_location b WHERE ref_id <> 0 AND y.mstr_ref_id = x.ref_id AND y.status='1' AND x.org_nm='$orgNameVls' AND y.fullname LIKE '%$empName%' AND x.department_id=a.id AND x.location_id=b.id)";
...
function empSalary($con, $empIds, $b_on){
 ...
 // ORDER BY z.id DESC LIMIT 1
    ...
    $qry = "SELECT x.id as empID, y.fullname as name, z.ctc_pay as totalctc, z.gross_pay as grs, z.basic_pay as basics, z.net_pay as netpay FROM hr_employee_service_register x, mstr_emp y, hr_employee_salary_report z WHERE ref_id = 0 AND x.emp_name = y.id AND y.status='1' AND y.id='$empIds' AND y.id=z.emp_id UNION (SELECT x.id as empID, x.emp_name as name, z.ctc_pay as totalctc, z.gross_pay as grs, z.basic_pay as basics, z.net_pay as netpay FROM hr_employee_service_register x, mstr_emp y, hr_employee_salary_report z WHERE ref_id <> 0 AND y.mstr_ref_id = x.ref_id AND y.status='1' AND y.id='$empIds' AND y.id=z.emp_id)";
    // return $qry;
    // exit();
...
if ($action=='empBonusRequestUpdate') {
    ...
    $insert_stmt = "INSERT INTO hr_bonus_request_edit(`b_id`, `br_id`, `org_id`, `b_type`, `b_mode`, `b_on`, `based_on`, `b_per`, `b_msg`, `b_status`, `stage_no`, `b_status_dt`, `created_by`, `remarks`, `br_created_at`) VALUES('$b_ids', '$hiddenIds', '$org_ids', '$b_types', '$b_modes', '$b_ons', '$based_ons', '$b_pers', '$b_msgs', '$b_statuss', '$stage_nos', '$b_status_dts', '$created_bys', '$remarkss', '$created_ons')";
