...
function saveIdsVls(slNo, ids, keys){
    ...
    if (slNo=='All') {
        ...
        if (getEdit!=undefined && getEdit!='') { // Edit Part
            if (listSlId==true) {
                // console.log('listSlId :- '+saveEditIdCheck);
                if (saveEditIdCheck!='') {
                    saveEditIdCheck += ', '+ids;
                    $('#getEditVlsChk_'+keys).val(saveEditIdCheck);
                }else{
                    $('#getEditVlsChk_'+keys).val(ids);
                    var saveEditIdCheck_a = saveEditIdCheck.split(',');
                    if (saveEditIdCheck_a[1]!=undefined && saveEditIdCheck_a[1]!='') {
                        var datas = '';
                        var counter=0;
                        for (var i = 0; i < saveEditIdCheck_a.length; i++) {
                            counter=i+1;
                            if (saveEditIdCheck_a[i].trim()!=listSlIdVls.trim()) {
                                if (saveEditIdCheck_a.length==counter) {
                                    datas += saveEditIdCheck_a[i];
                                }else{
                                    datas += saveEditIdCheck_a[i]+', ';
                                }
                            }
                        ...
                        if (saveEditIdCheck_a[0]==saveEditIdCheck) {
                            $('#getEditVlsChk_'+keys).val('');
                        }else{
                            $('#getEditVlsChk_'+keys).val(ids);
                        }
                ...
                if (saveEditId!='') {
                    ...
                    if (saveEditIdSpl[1]!=undefined && saveEditIdSpl[1]!='') {
                        ...
                        for (var i = 0; i < saveEditIdSpl.length; i++) {
                            counter=i+1;
                            if (saveEditIdSpl[i].trim()!=listSlIdVls.trim()) {
                                if (saveEditIdSpl.length==counter) {
